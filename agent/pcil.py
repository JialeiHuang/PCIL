...
class PCIL:
    def __init__(self, obs_shape, action_shape, device, lr, feature_dim,
                 ...
                 update_every_steps, stddev_schedule, stddev_clip, use_tb, T,
        ...
        actor_dim = repr_dim
        # if use_actions:
        #     repr_dim = repr_dim + action_shape[0]

        self.discriminator = Discriminator(disc_dim, hidden_dim).to(device)

        self.actor = Actor(actor_dim, action_shape, feature_dim,
    ...
    def update_expert_replay_iter(self, iter):
        self.expert_replay_iter = iter
    ...
    def update(self, replay_iter, expert_replay_iter, step, bc_regularize=False):
        # replay iter, expert_replay_iter
        ...
        results = self.update_contra_disc(obs, action, expert_obs,
                                            expert_action)
    ...
    def get_input(self, obses, actions):
        obses = torch.tensor(obses).to(self.device).float()
    ...
    def pc_rewarder(self, obses, actions):
        agent_inputs = self.get_input(obses, actions)
        expert_obses, expert_actions = utils.to_torch(next(self.expert_replay_iter),
                            self.device)
        expert_inputs = self.get_input(expert_obses, expert_actions)

        with torch.no_grad():
            with utils.eval_mode(self.discriminator):
                d = self.discriminator(agent_inputs)
                ed = self.discriminator(expert_inputs)
                ed = ed.unsqueeze(0).repeat(10,1,1).reshape(-1, d.shape[1])[:d.shape[0]]
                reward = (d * ed).sum(-1)
            return reward.flatten().detach().cpu().numpy()

    def update_contra_disc(self, policy_obs, policy_action, expert_obs,
                             expert_action):
        metrics = dict()
        ...
        if self.use_actions:
            ...
            disc_input = torch.cat([disc_input, disc_action], dim=1).float()


        disc_output = self.discriminator(disc_input)
        
        # change by jialei
        feature_expert = disc_output[:batch_size]
        feature_agent = disc_output[batch_size:]

        feature_expert_pos_idx = torch.randperm(feature_expert.shape[0]).to(self.device)
        feature_expert_pos = feature_expert[feature_expert_pos_idx]
        feature_agent_pos_idx = torch.randperm(feature_agent.shape[0]).to(self.device)
        feature_agent_pos = feature_agent[feature_agent_pos_idx]

        feature_expert_neg = feature_agent
        feature_agent_neg = feature_expert

        feature_agent_key = torch.cat((feature_agent_pos.unsqueeze(1), feature_agent_neg.unsqueeze(0).repeat(batch_size, 1, 1)), 1)
        feature_expert_key = torch.cat((feature_expert_pos.unsqueeze(1), feature_expert_neg.unsqueeze(0).repeat(batch_size, 1, 1)), 1)


        expert_logits = (feature_expert.unsqueeze(1).repeat(1, batch_size + 1, 1)) * feature_expert_key
        agent_logits = (feature_agent.unsqueeze(1).repeat(1, batch_size + 1, 1)) * feature_agent_key

        expert_logits = expert_logits.sum(-1)
        agent_logits = agent_logits.sum(-1)
        logits = torch.cat((expert_logits ,agent_logits), 0)
        ...
        pc_loss = F.cross_entropy(logits, labels, reduction='sum')
        # end change

        expert_obs, policy_obs = torch.split(disc_input, batch_size, dim=0)
        grad_pen = self.compute_gradient_penalty(self.discriminator, expert_obs,
                                            policy_obs)
        ...
        metrics['pc_loss'] = pc_loss.mean().item()
        metrics['acc_expert'] = acc_expert.mean().item()
    ...
    def compute_gradient_penalty(self, discriminator, expert_data, policy_data):
        ...
        with torch.no_grad():
            expert_disc = discriminator(expert_inputs)
        
        reward = (disc * expert_disc).sum(-1)

        ones = torch.ones(reward.size()).to(disc.device)
        grad = autograd.grad(outputs=reward,
                            inputs=mixup_data,
                            grad_outputs=ones,
                            create_graph=True,
                            retain_graph=True,

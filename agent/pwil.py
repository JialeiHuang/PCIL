...
class PWILAgent:
	def __init__(self, obs_shape, action_shape, device, lr, feature_dim,
				 ...
				 rewards, sinkhorn_rew_scale, update_target_every, name,
	...
	def pwil_rewarder(self, observations, demos, step):
		...
		demo = np.concatenate(demos, axis=0)
		...
		reward_sigma = 5 * obs.shape[0] / np.sqrt(obs.shape[-1])

		exp_weights = np.ones([exp.shape[0]]) / exp.shape[0]
		obs_weight = np.ones([obs.shape[0]]) / obs.shape[0]
		reward = np.zeros([obs.shape[0]])
		# ipdb.set_trace()
		for idx, cur_obs in enumerate(obs):
			# exp = torch.cat((exp, exp[-1].unsqueeze(0)))
			norms = np.linalg.norm(exp - cur_obs, axis=1)
			weight = obs_weight[idx] - 1e-8
			cur_cost = 0
			while weight > 0:
				argmin = norms.argmin()
				expert_weight = exp_weights[argmin]
				if weight >= expert_weight:
					weight -= expert_weight
					cur_cost += expert_weight * norms[argmin]
					exp_weights = np.delete(exp_weights, argmin, 0)
					exp = np.delete(exp, argmin, 0)
					norms = np.delete(norms, argmin, 0)
				else:
					cur_cost += weight * norms[argmin]
					exp_weights[argmin] -= weight
					weight = 0
			cur_reward = 5 * np.exp(-reward_sigma * cur_cost)
			reward[idx] = cur_reward
		return reward

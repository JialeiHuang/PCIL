...
class WandbLogger(object):
    def __init__(self, project_name, id, args, log_dir):
        self._log_dir = log_dir
        self._train_mg = MetersGroup(log_dir / 'train.csv',
                                    formating=COMMON_TRAIN_FORMAT)
        self._eval_mg = MetersGroup(log_dir / 'eval.csv',
                                    formating=COMMON_EVAL_FORMAT)
        self.writer = wandb.init(project=project_name, name=id)
        self.writer.config.update(args)
        self.step = 0
        self.exp_name = id

    def _wb_log(self, key, value, step):
        self.writer.log({key: value}, step=step)

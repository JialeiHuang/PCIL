...
class WorkspaceIL:
    def __init__(self, cfg):
        ...
        if repr(self.agent) != 'drqv2':
            ...
            self.cfg.expert_dataset, self.cfg.batch_size // self.cfg.bz_ratio, self.cfg.num_demos, self.cfg.obs_type)
    ...
    def setup(self):
        ...
        self.logger = Logger("pcil", self.cfg.exp_name, self.cfg, self.work_dir)
        # create envs
    ...
    def train_il(self):
        ...
        if repr(self.agent) == 'pcil' :
            self.agent.update_expert_replay_iter(self.expert_replay_iter)
        ...
        while train_until_step(self.global_step):
            if time_step.last():
                ...
                elif repr(self.agent) == 'pwil':
                    new_rewards = self.agent.pwil_rewarder(
                        observations, self.expert_demo, self.global_step)
                    new_rewards_sum = np.sum(new_rewards)
                elif repr(self.agent) == 'dac' :
                    new_rewards = self.agent.dac_rewarder(observations, actions)
                    new_rewards_sum = np.sum(new_rewards)
                elif repr(self.agent) == 'fairl' :
                    new_rewards = self.agent.fairl_rewarder(observations, actions)
                    new_rewards_sum = np.sum(new_rewards)
                elif repr(self.agent) == 'pcil':
                    new_rewards = self.agent.pc_rewarder(observations, actions)
                    new_rewards_sum = np.sum(new_rewards)
                ...
                if repr(self.agent) != 'drqv2' and repr(self.agent) != 'bc':
                    true_reward = np.array([time_steps[i].reward for i in range(len(time_steps))])
                    pseudo_reward = np.array(new_rewards)
                    cor = np.corrcoef(true_reward.reshape(-1), pseudo_reward.reshape(-1))[0,1]

                for i, elt in enumerate(time_steps):
                    elt = elt._replace(
                        observation=time_steps[i].observation[self.cfg.obs_type])
                    
                    if repr(self.agent) == 'potil'or repr(self.agent) =='pwil' or repr(self.agent) == 'dac'  or repr(self.agent) == 'fairl' or repr(self.agent) == 'pcil':
